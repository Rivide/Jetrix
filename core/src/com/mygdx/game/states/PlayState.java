...
public class PlayState extends State {
    ...
    public PlayState() {
        ...
        cam.setToOrtho(false, Jetrix.WIDTH, Jetrix.HEIGHT);
        cam.position.x = Jetrix.WIDTH / 2f;
        cam.position.y = Jetrix.HEIGHT / 2f;
        sr = new ShapeRenderer();
        sr.setProjectionMatrix(cam.combined);

        jet = new Jet(Jetrix.WIDTH / 2f - 32, 64, 0, 100, 20, 1);
        jetMouseOffsetY = 48;
        ...
        try {
            spawnEnemies.put(
                    new EnemyJet(0, Jetrix.HEIGHT, 225, 20,
                            new Instruction("moveTo", 0, 10, 0,
                                    0f, Jetrix.HEIGHT - 64f)
    ...
    public void render(SpriteBatch sb) {
        ...
        for (int x = 0; x < Jetrix.WIDTH; x += bg.getWidth()) {
            for (int y = 0; y < Jetrix.HEIGHT + Math.ceil(-bgOffset / 64) * 64; y += bg.getHeight()) {
